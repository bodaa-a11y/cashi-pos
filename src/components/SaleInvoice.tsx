      ...
      <aside className="w-full md:w-[32%] bg-white border-l border-stone-200 flex flex-col overflow-hidden shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2.5 bg-stone-50/50">
          {cart.length === 0 ? (
            ...
            <div className="py-24 text-center space-y-4">
              ...
              <p className="text-xs text-stone-400 max-w-xs mx-auto">أضف منتجات من قائمة المنيو المقابلة للبدء في إنشاء الفاتورة</p>
              <button
            ...
            cart.map((item, index) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  {/* Meta details */}
                  <div
                    onClick={() => handleOpenItemNotes(index)}
                    className="flex-1 cursor-pointer hover:opacity-85 pr-2"
                    title="اضغط لتعديل الملاحظات أو اختيار تعديل سريع"
                  >
        ...
        <div className="border-t border-stone-200 p-4 bg-stone-50 space-y-3 shrink-0">
          {cart.length === 0 ? (
            ...
            <div className="space-y-2 text-right text-xs">
              <div className="flex justify-between text-stone-500">
                ...
                <span className="font-bold font-mono">{subtotal.toFixed(2)} ر.س</span>
              </div>
              
              <div
                onClick={() => setShowDiscountModal(true)}
                className="flex justify-between text-red-600 cursor-pointer hover:bg-red-50/50 p-1 rounded transition-all"
                title="اضغط لتطبيق خصم"
              >
                <span className="flex items-center gap-1">
                  <Tag className="w-3.5 h-3.5" />
                  <span>الخصم {appliedDiscount.value > 0 ? `(${appliedDiscount.reason})` : ""}:</span>
                </span>
                <span className="font-bold font-mono">-{discountAmount.toFixed(2)} ر.س</span>
              </div>

              <div className="flex justify-between text-stone-500">
                <span>ضريبة القيمة المضافة (15%):</span>
                <span className="font-bold font-mono">{taxAmount.toFixed(2)} ر.s</span>
              </div>

              <div className="flex justify-between text-lg font-extrabold text-stone-800 border-t border-stone-200 pt-2">
                <span>الإجمالي الكلي:</span>
                <span className="font-mono text-[#2E7D32]">{grandTotal.toFixed(2)} ر.س</span>
              </div>
            </div>
          )}
          ...
          <button
            ...
            className="w-full mt-2.5 py-1.5 bg-stone-200 hover:bg-stone-300 text-stone-700 text-[10px] font-bold rounded-lg flex items-center justify-center gap-1"
          >
        ...
        <div className="bg-white border-b border-stone-200 p-4 shrink-0 flex flex-col md:flex-row gap-4 items-center justify-between">
          ...
          <div className="flex gap-2 w-full md:w-auto justify-end">
            ...
            <button
              ...
              title="تحديث قائمة الأصناف والمنيو"
            >
